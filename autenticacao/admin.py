...
def gerar_relatorio_pontos_pdf(modeladmin, request, queryset):
    ...
    p.drawCentredString(
        width / 2.0, height - 1.75 * inch, "Relatório de Frequência do Bolsista"
    )
    p.setFont("Helvetica-Bold", 16)
    ...
    pontos_para_calculo = Ponto.objects.filter(bolsista=servidor).order_by(
        "data_hora_do_ponto"
    )
    ...
    pontos_para_display = Ponto.objects.filter(bolsista=servidor).order_by(
        "-data_hora_do_ponto"
    )
